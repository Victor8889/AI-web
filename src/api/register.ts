 ...
 * @FilePath: \chatgpt\src\api\register.ts
 * @Description: 注册 相关接口
 */
...
export function verifyEode<T>(email: string, code: string, inviteCode: string) {
  return get<T>({
    url: `/user/verify_email_code?email=${email}&code=${code}&inviteCode=${inviteCode}`,
  })
}

export function sendCode<T>(identity: string) {
  return get<T>({
    url: `/user/send_forget_code?identity=${identity}`,
  })
}

export function resetPassword<T>(data: resetPasswordModel) {
  return post<T>({
