...
import { Moon, Notification, Present, Sunny } from '@element-plus/icons-vue'
    ...
    try {
      ...
      const data = reactive({
        ...
        menuList: [
          ...
          { menuId: 'Chat', menuName: '助手聊天', path: 'Chat', children: [] },
          { menuId: 'mj', menuName: '文/图', path: 'mj', children: [] },
          ...
          { menuId: 'video', menuName: 'Sora文生视频', path: 'video', children: [] },
          // { menuId: 'pic', menuName: '文生图', path: 'pic', children: [] },
        ],
...
<template>
  ...
  >
    ...
    <SidebarItem v-for="item in menuList" :key="item.menuId" :item="item" :collapse="collapse" style="height:6%" @click="navigateTo(item)">
      <template #icon />
    </SidebarItem>
    <div class="left-bottom">
      ...
      <div class="hover-button-container">
        <HoverButton title="公告" @click="showNotify = true">
          <span class=" text-[#4f555e] dark:text-white">
            <Notification class="navbar-icon" :class="nightMode ? 'sun-mode' : 'night-mode'" />
          </span>
        </HoverButton>
        <Notify v-model="showNotify" /><!-- Notify :grid-data="gridData" -->
      </div>
      <div class="hover-button-container">
        <HoverButton title="充值" @click="showPresent = true">
          <span class=" text-[#4f555e] dark:text-white">
