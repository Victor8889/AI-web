...
//混图
const fileList = ref([])
const fileList2 = ref([])//<UploadFileInfo[]>
...
async function uploadImgBlend(options: UploadCustomRequestOptions,pic:string) {
...
async function uploadImgBlend2(options: UploadCustomRequestOptions,pic:string) {
    const item = options.file
  try {
    if(checkIfUploaded2(item)){
      const index = fileList2.value.findIndex((f) => f.id === undefined);
      if (index !== -1) {
    ...
    // 图片格式大小信息没问题 执行上传图片的方法
      ...
      if (result.code === 200) {
        const files = result.data.split(',') // 原图路径
        imageUrls2.value.push(baseURL + files[0]);
        imageNames2.value.push(item.file.name);
        ms.success('上传成功')
        
      }
      else { 
       const index = fileList2.value.findIndex((f) => f.id === item.id)
      if (index !== -1) {
        fileList2.value.splice(index, 1)
      }
        ms.error('上传失败，请稍后重试') }
...
function checkIfUploaded2(item: UploadFileInfo): boolean {
  // 检查文件是否已经上传过
  const index = imageNames2.value.indexOf(item.file.name);
  return index !== -1;
}
...
function handleRemove2(item: UploadFileInfo){
const index = fileList2.value.findIndex((f) => f.id === item.file.id)
      
if(index > -1){
  imageNames2.value.splice(index,1)
  imageUrls2.value.splice(index,1)
}
}
// 点击---开始合成
async function sendClick() {
  ...
  console.info(imageNames.value.length)
  console.info(imageUrls2.value.length)
  console.info(imageNames2.value.length)
    try{
        if(imageUrls.value.length < 1){
          ms.error("请上传“你的头像")
          return
        }
        if(imageUrls2.value.length < 1){
          ms.error("请上传“明星图")
          return
        }
        if(isUploading.value){
      ...
      const pushData: modalImagineRequest = {
          ...
          roomId: 123,
          mjType:props.myProp,//普通速度
          actionName:'换脸：'+imageUrls.value+";"+imageUrls2.value,
          maskBase64:imageUrls.value+";"+imageUrls2.value,
          prompt:'',
      }
    await api.RoomMidjourneyCzp(pushData).then(() =>{
      fileList.value = []
    ...
    <main class="flex-1 overflow-hidden">
      <div id="scrollRef" ref="scrollRef" class="h-full overflow-hidden overflow-y-auto">
        ...
        >
          ...
          <div style=" margin-top:-1%; height: 15px;">
          ...
          :default-file-list="fileList"
            ...
            >
              <span >点击上传<br>你的头像</span>
                
              </NUpload> </div><div style="margin-top:1%; margin-left: 2%;">
              <apsn style="font-size: 36px;color:darkgray;">+</apsn></div><div style="margin-left: 2%;">
              <NUpload  
              ...
              :max="1"
              @before-upload="beforeUpload"
              :custom-request="uploadImgBlend2"
              :on-remove="handleRemove2"
            >
              <span>点击上传<br>明星图</span>
                
              </NUpload> </div>
            </div>
            <div style="margin-top:0.5%">
              <span style="color: grey;margin-top:1%">说明：<br>
                1 图片都必须包含脸，否则出不来图&nbsp;
                2 “明星图”可以先用mj绘画制作出来<br>
                3 “明星图”是动漫图也行&nbsp;
                4 “你的头像”建议用一寸个人照
